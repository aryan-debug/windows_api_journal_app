...
def _enum_child_proc(hwnd, lparam):
    user32.SendMessageW(
        hwnd, user32.WindowMessage.SETFONT, WPARAM(lparam), MAKELPARAM(1, 0)
    )
    return True
...
class Window:
    def __init__(
        ...
        result = user32.GetClassInfoW(
            self.msg_handler.module_handle, self.title, ctypes.byref(temp_class)
        )
        ...
        @user32.WNDPROC
        def __window_proc_fn(
            hwnd: HANDLE, umsg: user32.MSG, wparam: WPARAM, lparam: LPARAM
        ):
    ...
    def show(self):
        ...
        user32.EnumChildWindows(
            self.handle,
            user32.WNDENUMPROC(_enum_child_proc),
    ...
    def __window_proc(
        self, hwnd: HANDLE, umsg: user32.MSG, wparam: WPARAM, lparam: LPARAM
    ):
...
class Control:
    ...
    def __init__(
        ...
        self.handle = user32.CreateWindowExW(
            ...
            class_name,
            text,
            user32.WindowStyle.TABSTOP
            | user32.WindowStyle.VISIBLE
            | user32.WindowStyle.CHILD
            | styles,
            position[0],
            position[1],
            dimensions[0],
            ...
            ctypes.cast(
                user32.GetWindowLongPtrW(parent.handle, user32.GetWindowLong.HINSTANCE),
                HINSTANCE,
            ),
            None,
        )
...
class Label(Control):
    def __init__(
        ...
        super().__init__(
            parent, "STATIC", text, position, dimensions, 0, onclick=onclick
        )
